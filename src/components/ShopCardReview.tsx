...
export default function ShopCardReview({ shopId }: Props) {
  ...
  useEffect(() => {
    getReviewsByShopId(shopId).then((reviews) => {
      setCount(reviews.length);
      setAvg(getAverageRating(reviews));
      setLatestReview(reviews.length > 0 ? reviews[0] : null);
    });
  }, [shopId]);
  ...
  return (
    <div className="mt-3 pt-3 border-t border-stone-100">
        ...
        <div className="flex gap-0.5">
          {[1, 2, 3, 4, 5].map((s) => (
            <Star
              key={s}
              className={`w-3 h-3 ${
                s <= (avg ?? 0) ? "fill-amber-400 text-amber-400" : "text-stone-200"
              }`}
            />
          ))}
        </div>
      ...
      <p className="text-xs text-stone-500 line-clamp-2">
        <span className="font-medium text-stone-600">{latestReview.author_name}：</span>
