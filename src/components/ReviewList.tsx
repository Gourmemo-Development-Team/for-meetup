...
export default function ReviewList({ reviews }: Props) {
  if (reviews.length === 0) {
    return (
      <p className="text-sm text-stone-400 text-center py-6">まだレビューがありません。最初の一件を投稿してみましょう！</p>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {reviews.slice().reverse().map((review) => (
        <div key={review.id} className="bg-white rounded-xl border border-stone-100 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-stone-800 text-sm">{review.authorName}</span>
            </div>
